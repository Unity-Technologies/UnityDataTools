...
// Generates the ScriptableObject assets used as build sources. The dictionaries are populated before the assets
// are saved, so the entries are serialized into the asset files.
//
...
public static class GenerateAssets
{
    ...
    public static void Generate()
    {
        ...
        CreateReference("ContentDirectoryRoot", loadable, singleLoadable, serializationDemo);

        AssetDatabase.SaveAssets();

        Debug.Log("GenerateAssets: created leaf reference assets and AssetBundleRoot / ContentDirectoryRoot.");
    }
