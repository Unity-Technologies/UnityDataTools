...
LEFT JOIN content_layout_binary_artifacts ba ON ba.content_hash = f.content_hash
LEFT JOIN serialized_files sf ON sf.id = f.serialized_file;

-- Source asset -> the file(s) it was built into.
...
-- File-to-file dependency edges with names resolved on both sides.
CREATE VIEW IF NOT EXISTS content_layout_serialized_file_dependencies_view AS
SELECT d.serialized_file_index, src.content_hash || '.cf' AS filename, d.position,
       d.dependency_index, dep.content_hash || '.cf' AS dependency_filename, dep.cfid AS dependency_cfid
FROM content_layout_serialized_file_dependencies d
INNER JOIN content_layout_serialized_files src ON src.file_index = d.serialized_file_index
INNER JOIN content_layout_serialized_files dep ON dep.file_index = d.dependency_index;
...
FROM content_layout_serialized_files f
INNER JOIN content_layout_binary_artifacts ca ON ca.content_hash = f.content_hash
